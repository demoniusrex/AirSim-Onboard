...
#ifndef msr_airlib_vehicles_OnboardMultiRotor_hpp
#define msr_airlib_vehicles_OnboardMultiRotor_hpp
...
class OnboardMultiRotor : public MultiRotorParams {
public:
    OnboardMultiRotor(const AirSimSettings::VehicleSettings& vehicle_settings, std::shared_ptr<const SensorFactory> sensor_factory)
        : sensor_factory_(sensor_factory)
    {
        connection_info_ = getConnectionInfo(vehicle_settings);
    }

    virtual ~OnboardMultiRotor() = default;
...
protected:
    ...
    {
        unique_ptr<DroneControllerBase> controller(new OnboardDroneController());
        auto onboard_controller = static_cast<OnboardDroneController*>(controller.get());
        onboard_controller->initialize(connection_info_, & getSensors(), true);
...
private:
    ...
    static OnboardDroneController::ConnectionInfo getConnectionInfo(const AirSimSettings::VehicleSettings& vehicle_settings)
    {
        Settings child;
...
private:
    OnboardDroneController::ConnectionInfo connection_info_;
